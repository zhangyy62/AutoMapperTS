...
class AutoMapper extends AutoMapperBase {
    ...
    public createMap(sourceKeyOrType: string | (new () => any), destinationKeyOrType: string | (new () => any)): ICreateMapFluentFunctions {
    ...
    public map(sourceKeyOrType: stringOrClass, destinationKeyOrType: stringOrClass, sourceObject: any): any {
        if (arguments.length === 3) {
            return this.mapInternal(this.getMapping(this.mappings, sourceKeyOrType, destinationKeyOrType), sourceObject);
        }

        // provide performance optimized (preloading) currying support.
        if (arguments.length === 2) {
            return (srcObj: any): any => this.mapInternal(this.getMapping(this.mappings, sourceKeyOrType, destinationKeyOrType), srcObj);
        }
    ...
    private createMappingObjectForGivenKeys(srcKeyOrType: string | (new () => any), dstKeyOrType: string | (new () => any)): IMapping {
        var mapping: IMapping = {
            sourceKey: this.getKey(srcKeyOrType),
            destinationKey: this.getKey(dstKeyOrType),
    ...
    private createMapGetFluentApiFunctions(mapping: IMapping): ICreateMapFluentFunctions {
        ...
        var fluentFunc: ICreateMapFluentFunctions = {
            ...
            ignoreAllNonExisting: (): ICreateMapFluentFunctions => this.createMapIgnoreAllNonExisting(mapping, fluentFunc),
            convertToType: (type: new () => any): ICreateMapFluentFunctions => this.createMapConvertToType(mapping, fluentFunc, type),
